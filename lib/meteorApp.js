...
export default class MeteorApp {
    ...
    buildServerTarget() {
        return new Promise((resolve, reject) => {
            ...
            let errorTimeout = null;
            let killTimeout = null;
            let serverBuildCheckInteval = null;
            ...
            function clearTimeoutsAndIntervals() {
                clearInterval(serverBuildCheckInteval);
                clearTimeout(buildTimeout);
                clearTimeout(errorTimeout);
                clearTimeout(killTimeout);
            }

            const args = ['build', this.$.env.paths.meteorApp.webServerPro, '--server-only', '--directory'];
            ...
            function copyBuild() {
                self.log.info('fullStack: coping server bundle');
                self.copyBuild().then(() => {
                    exit();
                }).catch(() => {
                    error('copy');
            ...
            function runNpm() {
                self.log.info('fullStack: installing produccion modules');
                return self.runNpm(['install', '--production'], 'ignore', join(self.$.env.paths.meteorApp.webServerProBundle, 'programs', 'server'));
            }

            this.log.info('fullStack: starting server bundle build');
            serverBuildCheckInteval = setInterval(() => {
                // Check if we already have cordova-build ready.
                if (this.$.utils.exists(this.$.env.paths.meteorApp.webServerProBundle)) {
                    clearInterval(serverBuildCheckInteval);
            ...
            child.stderr.on('data', (chunk) => {
                ...
                if (
                    !~line.indexOf('WARNING: The output directory is under your source tree.') &&
                    !~line.indexOf('Node#moveTo was deprecated')
                ) {
                    // We will exit 1s after last error in stderr.
                    errorTimeout = setTimeout(() => {
                        error('error');
     ...
     */
    async copyBuild() {
        if (this.$.env.options.fullStack) {
            await this.copyFullStackBuild();
        } else {
            await this.copyCordovaBuild();
        }
    }

    async copyFullStackBuild() {
        this.log.debug('fullstack: clearing build dir');
        try {
        ...
        const serverBuild = this.$.env.isProductionBuild() ? this.$.env.paths.meteorApp.webServerProBundle : this.$.env.paths.meteorApp.webServer;

        if (!this.$.utils.exists(serverBuild)) {
            this.log.error(`fullstack: no server build found at ${serverBuild}`);
            this.log.error('fullstack: are you sure you did run meteor?');
            throw new Error('fullstack: required file not present');
        }

        this.log.verbose('fullstack: copying server build');
        shell.cp(
            '-R', `${serverBuild}`, this.$.env.paths.electronApp.meteorApp
        );

        shell.cp(join(this.$.env.paths.meteorApp.bin, 'node*'), this.$.env.paths.electronApp.root);
        ...
        this.log.info('server build copied to electron app');

        this.log.info('try to remove temp server bundle');
        await this.$.utils.rmWithRetries('-rf', this.$.env.paths.meteorApp.webServerPro);
    }

    async copyCordovaBuild() {
     ...
     */
    async build() {
        if (this.$.env.options.fullStack && this.$.env.isProductionBuild()) {
            this.log.info('fullStack: building meteor server bundle');
            try {
                await this.buildServerTarget();
            } catch (reason) {
                this.log.error('error occurred during building server target', reason);
            }
        } else {
        ...
        // TODO: How do this in fullStack build
        if (!this.$.env.options.fullStack) {
