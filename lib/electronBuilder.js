...
export default class InstallerBuilder {
    ...
    afterPack(context) {
        return new Promise(async (resolve, reject) => {
            ...
            const packagedAppPath = this.getPackagedAppPath(context);
            ...
            if (this.$.utils.exists(this.$.env.paths.electronApp.extractedNodeModules)) {
                ...
                shell.cp(
                    ...
                    path.join(packagedAppPath, 'node_modules')
                );
            }

            if (this.$.env.options.fullStack) {
                this.log.debug('fullstack: injecting web server node_modules');
                shell.cp(
                    '-Rf',
                    path.join(this.$.env.paths.electronApp.meteorApp, 'programs', 'server', 'node_modules'),
                    path.join(packagedAppPath, 'meteor', 'programs', 'server', 'node_modules')
                );

                this.log.debug('fullstack: injecting web server npm folder');
                shell.cp(
                    '-Rf',
                    path.join(this.$.env.paths.electronApp.meteorApp, 'programs', 'server', 'npm'),
                    path.join(packagedAppPath, 'meteor', 'programs', 'server', 'npm')
                );
            }
    ...
    async build() {
        this.settings = this.$.desktop.getSettings();
        const { settings } = this;
